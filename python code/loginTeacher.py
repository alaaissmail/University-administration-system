...
#-------------------password------------
    ...
    def get_name(self) :
        ...
        cur.execute("select teacher from teacher_table where teacher= %s LIMIT 1",(teacher_name,))
        u=cur.fetchone()
        name = u[0]            
        lo.commit()
        lo.close() 
    ...
    def get_pass(self) :
        ...
        global password
        teacher_name = self.inter.get()
        cur.execute("select password from teacher_table where teacher= %s LIMIT 1",(teacher_name,))
        u=cur.fetchone()
        password = u[0]           
        lo.commit()
        lo.close() 
...
#-----------login app----------- 
    def inapp(self):
            if self.inter.get() == self.get_name() and self.passinter.get() == self.get_pass():
                ...
                bomb = teacher432.TEacher(self.inter.get())
